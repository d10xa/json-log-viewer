...
class CsvLogLineParserTest extends FunSuite {
  ...
  test("parse CSV log line with standard headers") {
    val headerLine = "@timestamp,level,message,logger_name,thread_name,stack_trace"
    ...
    assertEquals(result.parsed.flatMap(_.timestamp), Some("2023-01-01T12:00:00Z"))
  ...
  test("parse CSV log line with custom headers") {
    ...
    assertEquals(result.parsed.flatMap(_.level), Some("INFO"))
    assertEquals(result.parsed.flatMap(_.message), Some("Test message"))
    assertEquals(result.parsed.exists(_.otherAttributes.contains("custom_field")), true)
    assertEquals(result.parsed.flatMap(_.otherAttributes.get("custom_field")), Some("custom value"))
  }

  test("parse CSV log line with spaces in header") {
    val headerLine = "\"@timestamp\",\"log level\",\"message text\",\"logger name\""
    ...
    assertEquals(result.parsed.exists(_.otherAttributes.contains("log level")), true)
    assertEquals(result.parsed.flatMap(_.otherAttributes.get("log level")), Some("INFO"))
    assertEquals(result.parsed.exists(_.otherAttributes.contains("message text")), true)
    assertEquals(result.parsed.flatMap(_.otherAttributes.get("message text")), Some("Test message"))
    assertEquals(result.parsed.exists(_.otherAttributes.contains("logger name")), true)
    assertEquals(result.parsed.flatMap(_.otherAttributes.get("logger name")), Some("Logger"))
  }

  test("parse CSV log line with missing fields") {
    val headerLine = "@timestamp,level,message,logger_name,thread_name,stack_trace,custom_field"
    val logLine = "2023-01-01T12:00:00Z,INFO,Test message,Logger"
    ...
    assertEquals(result.parsed.exists(_.otherAttributes.contains("custom_field")), false)
  }
